...
在**应用管理**页面，找到**模型配置**，默认会为您配置[UModelVerse](https://docs.ucloud.cn/modelverse/README) 服务，集成各大主流大模型API能力，可直接选择API Key并选择需要接入OpenClaw的大模型。如需更详细的模型管理能力可点击跳转到[ModelVerse 控制台](https://console.ucloud.cn/modelverse/experience/api-keys)或参考[UModelVerse产品文档](https://docs.ucloud.cn/modelverse/README)。
...
## OpenClaw WebUI
若您想要在OpenClaw原生面板（Web UI）与OpenClaw聊天，您可以在**应用管理**页面找到**安全访问OpenClaw面板**。
![image](/images/appconf_webui1.png)
安全访问将为您随机开放OpenClaw端口并生成管理员token，随后可点击链接访问OpenClaw面板。
