...
![image](/images/feishu_qrcode.png)
...
1. 在应用管理的**通道配置**区域，点击**添加**按钮，选择**飞书**通道。
2. 在弹出的配置框中，输入前面步骤中查询到的飞书机器人的 App ID 和 App Secret，点击**确认**完成添加。
![image](/images/appconf_model2.png)
3. 等待几十秒后查看状态，确保你添加的飞书通道显示**运行中**。
...
#### 订阅方式
回到飞书应用管理页，左侧导航栏点击 **事件与回调** ，点击进入页面。在事件配置页签，**订阅方式**选择**长连接接收事件**，点击保存。
...
若您期望将飞书机器人添加进聊天群组中使用，可以参考前述步骤继续添加更多群组相关权限，主要包括“消息已读”、“机器人进群”、“机器人被移出群”。

### 回调配置

#### 订阅方式
在“事件与回调-回调配置”页面中，**订阅方式**选择 “使用长连接接收回调”，点击保存，无需填写其他地址，配置自动生效。
...
## 配对操作
**提示**：如果首次对话提示如下图所示的配对请求。
![image](/images/feishu_peidui1.png)

可以复制机器人回复的最后一行命令行，从控制台登陆到部署openclaw的轻量应用云主机，将命令行粘贴并按回车执行，待命令执行后会提示配对成功。
